...
```{r}
rf_test_acc <- augment(reg_tree_final_fit1, new_data = uc_test) %>% 
  accuracy(truth = model, estimate = .pred_class)
rf_test_acc

augment(reg_tree_final_fit1, new_data = uc_test) %>% 
  roc_auc(model, .pred_Accord:.pred_Silverado_1500) 
