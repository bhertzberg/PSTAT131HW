...
```{r}
reg_grid <- grid_regular(mtry(range = c(1,8)), trees(range = c(10, 300)), min_n(range = c(2,12)), levels = 2)
```
...
```{r}
best_rf1 <- select_best(rf_res, metric = "roc_auc")
...
```{r}
collect_metrics(rf_res2) %>% arrange(desc(mean))
```

```{r}
best_rf <- select_best(rf_res2, metric = "rmse")
