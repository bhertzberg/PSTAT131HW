...
With the data in a more manageable size, I filled all blank spaces with 'NA', which lets R know that there is no data contained. Then I removed all observations containing 'NA'.
```{r, eval = FALSE}
set.seed(619)
...
```{r}
collect_metrics(uc_svm_res) %>% arrange(desc(mean))
...
## Price Models (Regression)

```{r}
uc_rec2 <- recipe(price ~ model + miles + year +drivetrain+engine_size+transmission, data = uc_train) %>% 
  step_dummy(all_nominal_predictors()) %>% 
  step_normalize(all_predictors())
```
...
```{r}
collect_metrics(elastic_tune_res2) %>% arrange(desc(mean))

```

```{r}
best_elastic_mod2 <- select_best(elastic_tune_res2, metric = 'rsq')
uc_elastic_final2 <- finalize_workflow(net_wkflw2, best_elastic_mod2)
uc_elastic_final_fit2 <- fit(uc_elastic_final2, data = uc_train)
```


```{r}
# augment(uc_elastic_final_fit2, new_data = uc_train) %>% 
...
uc_pred <- predict(uc_elastic_final_fit2, uc_train) %>% 
  bind_cols(uc_train %>% dplyr::select(price))
multi_metric(uc_pred, truth = price, estimate = .pred)
```
...
```{r}
reg_grid <- grid_regular(mtry(range = c(1,8)), trees(range = c(10, 300)), min_n(range = c(2,12)), levels = 3)
```
...
write_rds(rf_res2, file = 'SavedModels/uc_rf_tuned_res.rds')
```

```{r}
uc_rf_reg_res <- read_rds(file = 'SavedModels/uc_rf_tuned_res.rds')
```


```{r}
collect_metrics(uc_rf_reg_res) %>% arrange(desc(mean))
```

```{r}
best_rf <- select_best(uc_rf_reg_res, metric = "rmse")

reg_tree_final <- finalize_workflow(rf_wf, best_rf)

reg_tree_final_fit <- fit(reg_tree_final, data = uc_train)
```

```{r}
augment(reg_tree_final_fit, new_data = uc_train) %>%
